...
template<typename F>
constexpr auto or_else_as(const Some &s, F f) {
  return f(s);
}
...
TEST_CASE("concept") {
  ...
  STATIC_REQUIRE(or_else.invocable<Some, F>);
  STATIC_REQUIRE(or_else.invocable<int &, F> == false);
}

TEST_CASE("pipe-operator") {
  constexpr Some s0{.value = 10};
  constexpr auto s1 = s0 | or_else([](auto s) {
                        return s.value + 10;
                      });
